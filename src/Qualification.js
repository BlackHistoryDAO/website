...
const Qualification = () => {
    ...
    useEffect(() => {
        const queryNum = async () => {
            const result = await api.query.bhdao.qualificationVotesCount();
        ...
        const queryVotes = async() => {
            if (titems > 0) {
                ...
                for (let i = 1; i <= titems; ++i) {
                    const result11 = await api.query.bhdao.qualificationVotes(i)
    ...
    const onYesButtonClick = async (e) => {
        ...
        let vote = true
        const call1 = api.tx.bhdao.castQualificationVote(e.target.value,vote)
    ...
    const onFinalizeClick = async (e) => {
        const fromAcct = await getFromAcct()
        console.log(e.target.value)
        const call1 = api.tx.bhdao.finalizeQualificationVoting(e.target.value)
        console.log(fromAcct)
        const unsub = await call1.signAndSend(fromAcct, (result) => {console.log(result.toHuman())})
        console.log(unsub)
    }
